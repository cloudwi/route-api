    ...
    class PlacesController < ApplicationController
      ...
      def like
        if @place.liked_by?(current_user)
          render json: { error: "이미 좋아요한 장소입니다" }, status: :unprocessable_entity
          return
        end
      ...
      # 좋아요 취소
      def unlike
        like = @place.place_likes.find_by(user: current_user)
        ...
        if like
          ...
          render json: { error: "좋아요하지 않은 장소입니다" }, status: :unprocessable_entity
        end
      end
